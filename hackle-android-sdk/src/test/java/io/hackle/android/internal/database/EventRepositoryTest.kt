...
internal class EventRepositoryTest {
    ...
    fun before() {
        ...
        every { databaseHelper.read(any<(SQLiteDatabase) -> Any>()) } answers {
            firstArg<(SQLiteDatabase) -> Any>().invoke(db)
        }
        every { databaseHelper.write(any<(SQLiteDatabase) -> Any>()) } answers {
            firstArg<(SQLiteDatabase) -> Any>().invoke(db)
        }
        every { databaseHelper.runInTransaction(any<(SQLiteDatabase) -> Any>()) } answers {
            firstArg<(SQLiteDatabase) -> Any>().invoke(db)
        }
    ...
    @Test
    fun `delete - 입력받은 count 만큼 오래된 이벤트를 삭제한다`() {
        // given
        val statement = mockk<SQLiteStatement>(relaxUnitFun = true)
        every { statement.simpleQueryForLong() } returns 42
        every { db.compileStatement(any()) } returns statement
