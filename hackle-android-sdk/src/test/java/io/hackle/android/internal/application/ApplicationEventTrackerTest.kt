...
class ApplicationEventTrackerTest {
    ...
    fun `onInstall should track install event when no previous version exists`() {
        ...
        val tracker = ApplicationEventTracker(userManager, core, packageInfo)
        ...
        expectThat(eventSlot.captured.properties.containsKey("previousVersionName")).isEqualTo(false)
        expectThat(eventSlot.captured.properties.containsKey("previousVersionCode")).isEqualTo(false)
    ...
    fun `onUpdate should handle null previousVersionInfo`() {
        ...
        expectThat(eventSlot.captured.properties["versionName"]).isEqualTo("1.0.0")
        expectThat(eventSlot.captured.properties["versionCode"]).isEqualTo(1L)
        expectThat(eventSlot.captured.properties["previousVersionName"]).isEqualTo(null)
        expectThat(eventSlot.captured.properties["previousVersionCode"]).isEqualTo(null)
    }
    ...
    fun `onForeground should track app open event`() {
        ...
        expectThat(eventSlot.captured.properties["isFromBackground"]).isEqualTo(true)
    }
