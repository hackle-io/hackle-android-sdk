...
import io.hackle.android.internal.user.local.*
...
class LocalUserManagerTest {
    ...
    fun `initialize - with saved user`() {
        repository.putString(
            "user",
            LocalUserManager.UserModel.from(
                User.builder()
    ...
    fun `syncIfNeeded - when no new identifier then do not sync cohort and sync target event`() {
        ...
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").build(),
                new = User.builder().id("id").build()
            )
        )
        // cohort not sync and target sync
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").deviceId("device_id").build(),
                new = User.builder().id("id").build()
            )
        )
        // cohort not sync and target event not sync
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").deviceId("device_id").build(),
                new = User.builder().id("id").deviceId("device_id").build()
            )
        )
        // cohort not sync and target event not sync
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").deviceId("device_id").identifier("custom", "custom_id").build(),
                new = User.builder().id("id").deviceId("device_id").build()
            )
        )
        // cohort not sync and target event not sync
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").deviceId("device_id").identifier("custom", "custom_id").build(),
                new = User.builder().id("id").deviceId("device_id").identifier("custom", "custom_id").build()
            )
        )
    ...
    fun `syncIfNeeded - when has new identifier then sync cohort and target event`() {
        ...
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().build(),
                new = User.builder().id("new_id").build()
            )
        )
        // sync cohort and not sync target event
        sut.syncIfNeeded(
        ...
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").build(),
                new = User.builder().id("id").deviceId("new_device_id").build()
            )
        )
        // sync cohort and sync target event
        sut.syncIfNeeded(
        ...
        sut.syncIfNeeded(
            UserUpdated(
                old = User.builder().id("id").deviceId("device_id").build(),
                new = User.builder().id("id").deviceId("device_id").identifier("custom", "new_custom_id").build()
            )
        )
        // sync cohort and not sync target event
        sut.syncIfNeeded(
