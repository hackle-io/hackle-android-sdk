...
class EventDispatcherTest {
    ...
    @RelaxedMockK
    private lateinit var dispatchExecutor: ExecutorService
    ...
    fun `dispatchExecutor 로 EventDispatchTask 를 실행시킨다`() {
        val sut = EventDispatcher(
            "http://localhost", eventRepository, eventExecutor, dispatchExecutor, httpClient)
        sut.dispatch(emptyList())
        verify(exactly = 1) {
            dispatchExecutor.submit(withArg {
    ...
    @Test
    fun `EventDispatchTask 실행 요청에 실패해도 에러가 발생하지 않는다`() {
        val sut = EventDispatcher(
            "http://localhost", eventRepository, eventExecutor, dispatchExecutor, httpClient)
        every { dispatchExecutor.submit(any()) } answers { throw RejectedExecutionException() }
    ...
    fun `dispatch - 이벤트 전송에 성공하면 해당 이벤트를 DB 에서 지운다`() {
        ...
        val sut = EventDispatcher(
            "http://localhost",
            eventRepository,
            mockRun(eventExecutor),
            mockRun(dispatchExecutor),
            httpClient
        )
        ...
        verify(exactly = 1) { eventExecutor.submit(withArg { expectThat(it).isA<EventDispatcher.DeleteEventTask>() }) }
    }
    ...
    fun `dispatch - 이벤트 전송에 실패하면 재시도를 위해 다시 PENDING 상태로 변경한다`() {
        ...
        verify(exactly = 1) { eventExecutor.submit(withArg { expectThat(it).isA<EventDispatcher.UpdateEventTask>() }) }
    }
    ...
    fun `DeleteEventTask - 이벤트를 모두 삭제한다`() {
        // then
        val events = listOf(EventEntity(1, FLUSHING, TRACK, "body"))
        val sut = EventDispatcher("http://localhost",
            eventRepository,
            eventExecutor,
            dispatchExecutor,
            httpClient
        ).DeleteEventTask(events)
    ...
    private fun mockRun(executor: ExecutorService): ExecutorService {

        every { executor.submit(any()) } answers { completedFuture(firstArg<Runnable>().run()) }
        return executor
    }
}