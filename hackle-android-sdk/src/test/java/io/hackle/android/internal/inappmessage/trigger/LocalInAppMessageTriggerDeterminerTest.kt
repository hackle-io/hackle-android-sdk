...
class LocalInAppMessageTriggerDeterminerTest {
    ...
    @InjectMockKs
    private lateinit var sut: LocalInAppMessageTriggerDeterminer
    ...
    fun `매칭되고 eligible한 inAppMessage가 없으면 null을 리턴한다`() {
        // given
        val messages = listOf(
            InAppMessages.config(id = 1, key = 1),
            InAppMessages.config(id = 2, key = 2),
        )
        every { workspaceManager.workspace(any()) } returns Workspaces.config(inAppMessages = messages)
    ...
    fun `매칭되고 eligible한 첫번째 inAppMessage를 트리거한다`() {
        ...
        val messages = listOf(
            InAppMessages.config(id = 1, key = 1),
            InAppMessages.config(id = 2, key = 2),
            InAppMessages.config(id = 3, key = 3),
            InAppMessages.config(id = 4, key = 4),
        )
        every { workspaceManager.workspace(any()) } returns Workspaces.config(inAppMessages = messages)
