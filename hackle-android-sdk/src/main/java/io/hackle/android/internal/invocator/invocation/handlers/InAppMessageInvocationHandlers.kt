...
import io.hackle.android.internal.invocator.model.InAppMessageEventDto
...
internal class HandleInAppMessageViewInvocationHandler(
    ...
    override fun invoke(request: InvocationRequest): InvocationResponse<Unit> {
        ...
        val handleTypes = dto.handleTypes.map { InAppMessageEventHandleType.valueOf(it) }
        view.handle(event, handleTypes)
        return InvocationResponse.success()
    }

    private fun inAppMessageEvent(view: InAppMessageView, dto: InAppMessageEventDto): InAppMessageEvent {
        return when (InAppMessageEvent.Type.valueOf(dto.type)) {
            InAppMessageEvent.Type.ACTION -> actionEvent(view, dto)
            InAppMessageEvent.Type.IMPRESSION,
            InAppMessageEvent.Type.CLOSE,
            InAppMessageEvent.Type.IMAGE_IMPRESSION -> throw IllegalArgumentException("Unsupported InAppMessageEvent.Type [${dto.type}]")
        }
    }

    private fun actionEvent(view: InAppMessageView, dto: InAppMessageEventDto): InAppMessageEvent.Action {
        val action = requireNotNull(dto.action)
        val element = requireNotNull(dto.element)
        return InAppMessageEvent.action(
            view = view,
