...
import io.hackle.android.internal.database.EventEntity.Companion.EVENT_COLUMN_NAME
...
internal class EventRepository(
    ...
    fun count(status: EventEntity.Status? = null): Long {
        return try {
            databaseHelper.read { db -> count(db, status) }
        } catch (e: Exception) {
    ...
    fun save(userEvent: UserEvent) {
        try {
            databaseHelper.write { db -> save(db, userEvent) }
        } catch (e: Exception) {
    ...
    private fun save(db: SQLiteDatabase, userEvent: UserEvent) {
        ...
        values.put(EVENT_COLUMN_NAME, userEvent.toDtoJson())
        db.insert(TABLE_NAME, null, values)
    }

    fun getEventsToFlush(limit: Long): List<EventEntity> {
        return try {
            databaseHelper.runInTransaction { db -> getEventsToFlush(db, limit) }
        } catch (e: Exception) {
    ...
    private fun getEventsToFlush(db: SQLiteDatabase, limit: Long): List<EventEntity> {
        return getEvents(db, limit)
            .also { update(db, it, FLUSHING) }
    }

    private fun getEvents(db: SQLiteDatabase, limit: Long): List<EventEntity> {
        val query =
            "SELECT * FROM $TABLE_NAME WHERE $STATUS_COLUMN_NAME = ${PENDING.code} ORDER BY $ID_COLUMN_NAME ASC LIMIT $limit"
    ...
    fun update(ids: List<EventEntity>, status: EventEntity.Status) {
        try {
            databaseHelper.runInTransaction { db -> update(db, ids, status) }
        } catch (e: Exception) {
    ...
    fun delete(events: List<EventEntity>) {
        ...
        databaseHelper.write {
            it.delete(TABLE_NAME, whereClause, null)
        }
    }

    fun delete(count: Long) {
        return try {
            databaseHelper.runInTransaction { db -> delete(db, count) }
        } catch (e: Exception) {
            log.error { "Failed to delete events: $e" }
        }
    }

    private fun delete(db: SQLiteDatabase, count: Long) {
...
private val UserEvent.type: EventEntity.Type
    get() {
        return when (this) {
...
private fun UserEvent.toDtoJson(): String {
    return when (this) {
