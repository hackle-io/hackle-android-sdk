...
internal class InAppMessageViewEventActorFactory(private val actors: List<InAppMessageViewEventActor<out InAppMessageViewEvent>>) {
...
internal class InAppMessageViewImpressionEventActor : InAppMessageViewEventActor<InAppMessageViewEvent.Impression> {
...
internal class InAppMessageViewActionEventActor(
    private val handlerFactory: InAppMessageActionHandlerFactory,
    ...
    override fun action(view: InAppMessageView, event: InAppMessageViewEvent.Action) {
        ...
        val handler = handlerFactory.get(event.action) ?: return
        handler.handle(view, event.action)
    }
}

internal class InAppMessageViewCloseEventActor : InAppMessageViewEventActor<InAppMessageViewEvent.Close> {
    override fun supports(event: InAppMessageViewEvent): Boolean {
